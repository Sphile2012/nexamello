...
export default function ProcessSection() {
  ...
  const examples = [
    { plan: "Starter", total: "R2,500", deposit: "R1,500", balance: "R1,000" },
    { plan: "Business", total: "R5,000", deposit: "R1,500", balance: "R3,500" },
    { plan: "Premium", total: "R9,000+", deposit: "R1,500", balance: "R7,500+" },
  ];
    ...
    <section id="workflow" className="py-24 sm:py-32 relative">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            >
              ...
              <div className="flex justify-center mb-6">
                <motion.div
                  ...
                  className={`w-16 h-16 rounded-2xl bg-gradient-to-br ${step.gradient} p-px shadow-xl`}
                >
              ...
              {/* Card */}
              <div className="rounded-2xl bg-card/60 border border-border/50 p-6 hover:border-primary/30 hover:bg-card transition-all duration-300">
                <div className="flex items-start justify-between mb-4">
                  <div>
                    <span className="text-xs font-inter font-bold text-primary uppercase tracking-widest">
                      Step {step.number}
                    </span>
                    <h3 className="font-space font-bold text-xl text-foreground mt-1">{step.phase}</h3>
                  </div>
                  <span className="text-xs font-inter font-semibold px-2.5 py-1 rounded-lg bg-secondary/60 text-muted-foreground">
                    {step.time}
                  </span>
                </div>
        ...
        <motion.div
          ...
          className="rounded-3xl bg-gradient-to-br from-primary/10 via-primary/5 to-transparent border border-primary/20 p-8 md:p-10"
        >
          <h3 className="font-space font-bold text-xl text-foreground text-center mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
            {examples.map((ex, i) => (
              <motion.div
                ...
                className="rounded-2xl bg-card/50 border border-border/40 p-5"
              >
                ...
                <div className="space-y-3">
                  <div className="flex justify-between items-center pb-2 border-b border-border/30">
                    ...
                    <span className="font-space font-bold text-foreground">{ex.deposit}</span>
                  </div>
                  <div className="flex justify-between items-center pb-2 border-b border-border/30">
                    <span className="text-sm font-inter text-muted-foreground">Website Total</span>
                    <span className="font-space font-bold text-foreground">{ex.total}</span>
                  </div>
                  <div className="flex justify-between items-center p-2.5 rounded-xl bg-primary/10">
                    ...
                    <span className="font-space font-bold text-primary">{ex.balance}</span>
                  </div>
                </div>
              </motion.div>
            ))}
          ...
          <p className="text-sm font-inter text-muted-foreground text-center mt-8 max-w-2xl mx-auto">
            The <span className="text-primary font-semibold">R1,500 booking fee is non-refundable</span> and secures your project slot. The remaining balance is due upon completion and approval before going live.
          </p>
        </motion.div>
      </div>
    </section>
